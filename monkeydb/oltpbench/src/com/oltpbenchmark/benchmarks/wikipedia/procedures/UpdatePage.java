...
public class UpdatePage extends Procedure {
	...
	public SQLStmt selectWatchList = new SQLStmt(
        "SELECT wl_user, wl_notificationtimestamp FROM " + WikipediaConstants.TABLENAME_WATCHLIST +
        " WHERE wl_title = ?" +
        "   AND wl_namespace = ?" +
		"   AND wl_user != ?"
    );
...
//			ps = this.getPreparedStatementReturnKeys(conn, insertText, new int[]{1});
			ps = this.getPreparedStatement(conn, insertText);
		param = 1;
...
//		ps = this.getPreparedStatementReturnKeys(conn, insertRevision, new int[]{1});
		ps = this.getPreparedStatement(conn, insertRevision);
		param = 1;
