...
#ifndef NEKARA_KEY_VALUE_STORE_COURSEWARE_H
#define NEKARA_KEY_VALUE_STORE_COURSEWARE_H
...
class courseware {
public:
    courseware(nekara::kv_store<std::string, web::json::value> *store,
...
courseware::courseware(nekara::kv_store<std::string, web::json::value> *store,
                       coyote::Scheduler *scheduler, bool coyote_enabled,
                       consistency consistency_level) {
    this->store = store;
...
void courseware::tx_start(bool delay) {
    if (!coyote_enabled) {
        if (delay)
...
#endif //NEKARA_KEY_VALUE_STORE_COURSEWARE_H
