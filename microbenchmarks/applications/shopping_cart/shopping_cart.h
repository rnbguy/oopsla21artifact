...
#ifndef NEKARA_KEY_VALUE_STORE_SHOPPING_CART_H
#define NEKARA_KEY_VALUE_STORE_SHOPPING_CART_H
...
class shopping_cart {
public:
    shopping_cart(user u, nekara::kv_store<std::string, web::json::value> *store,
...
shopping_cart::shopping_cart(user u, nekara::kv_store<std::string, web::json::value> *store,
                             coyote::Scheduler *scheduler, bool coyote_enabled,
                             consistency consistency_level) {
    this->user_id = u.id;
    this->store = store;
...
void shopping_cart::tx_start(bool delay) {
    if (!coyote_enabled) {
        if (delay)
...
std::vector<std::pair<item, int>> shopping_cart::get_cart_list(long session_id) {
    ...
    BOOST_LOG_TRIVIAL(info) << "CART " << session_id << " " << ss.str();
...
void shopping_cart::_add_item(int id, long session_id) {
    ...
    try {
        cart = this->store->get("cart:" + user, session_id);
    } catch (nekara::key_not_found_exception &e) {
...
#endif //NEKARA_KEY_VALUE_STORE_SHOPPING_CART_H
